"""
Comparison Script: Inception V3 - 2 Epochs
Dataset  : ../dataset  (11-class)
Model    : Inception V3 (ImageNet pretrained, input 299x299)
Output   : training/results/inceptionv3_2epochs.pth + inceptionv3_comparison.txt
"""
import os, time
import torch
...
SAVE_PATH   = os.path.join(RESULTS_DIR, "inceptionv3_2epochs.pth")
LOG_PATH    = os.path.join(RESULTS_DIR, "inceptionv3_comparison.txt")
BATCH_SIZE, EPOCHS, LR, VAL_SPLIT, SEED = 16, 2, 1e-4, 0.2, 42
...
train_tfms = transforms.Compose([transforms.RandomResizedCrop(299), transforms.RandomHorizontalFlip(), transforms.ColorJitter(0.2,0.2,0.2), transforms.ToTensor(), transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
val_tfms   = transforms.Compose([transforms.Resize(320), transforms.CenterCrop(299), transforms.ToTensor(), transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
...
try:    model = models.inception_v3(weights=models.Inception_V3_Weights.IMAGENET1K_V1)
except: model = models.inception_v3(pretrained=True)
...
best_acc  = 0.0
log_lines = [f"Model: {MODEL_NAME} | Device: {device} | Epochs: {EPOCHS} | Batch: {BATCH_SIZE} | Input: 299x299\nClasses: {class_names}\nTrain: {n_train} | Val: {n_val}\n\n"]
...
for epoch in range(EPOCHS):
    ...
    for x,y in train_loader:
        ...
        if isinstance(outputs, tuple):
            out, aux_out = outputs
            loss = criterion(out, y) + 0.4 * criterion(aux_out, y)
        else:
            out = outputs
    ...
    with torch.no_grad():
        for x,y in val_loader:
            ...
            _,p=torch.max(out,1); vc+=(p==y).sum().item(); vt+=y.size(0)
