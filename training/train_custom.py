...
# ─── Config ───────────────────────────────────────────────────────────────────
DATA_DIR    = "/Users/neal/Desktop/LATEST COO/coconut_dataset/train"
...
# ─── Dataset (Dynamic class discovery — no hardcoded class names) ─────────────
full_ds = datasets.ImageFolder(DATA_DIR)
class_names = full_ds.classes   # Auto-detected from folder names
...
# Save class mapping to JSON (index → class name)
...
best_acc  = 0.0
log_lines = [f"Training started | device={device} | epochs={EPOCHS} | batch={BATCH_SIZE} | lr={LR} | classes={num_classes}\n"]
...
summary += f"Classes ({num_classes}): {class_names}\n"
summary += f"Class map: {class_map_abs}\n"
