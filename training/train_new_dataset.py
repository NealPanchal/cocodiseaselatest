"""
Coconut Disease Detector — Retrain on New Dataset
==================================================
- Uses ./dataset  (6-class flat structure with nested images)
- Auto 80/20 train/val split (no pre-split needed)
- CUDA → MPS → CPU device selection
- Saves best model to mobilenet_new.pth (does NOT touch mobilenet_best.pth)
- Normalises class name: CCI_Caterpillars → Caterpillars  (keeps inference compat)
...
# Class-name normalisation map (new dataset → inference-compatible name)
CLASS_REMAP = {
...
raw_classes = full_ds.classes                       # e.g. ["CCI_Caterpillars", …]
class_names = [CLASS_REMAP.get(c, c) for c in raw_classes]

print(f"[INFO] Dataset folder   : {os.path.abspath(DATA_DIR)}")
print(f"[INFO] Raw class dirs   : {raw_classes}")
print(f"[INFO] Normalised names : {class_names}")
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_acc:
        ...
        log_lines.append(f"  [SAVED] Best model saved (val_acc={val_acc:.3f})\n")
...
print("\n[NEXT STEPS]")
print("  1. Review training_log.txt for accuracy comparison")
print("  2. Run: python switch_model.py --validate   (optional quick test)")
print("  3. Run: python switch_model.py --switch     (replace mobilenet_best.pth)")
print("  4. Run: python switch_model.py --rollback   (restore old model)")
